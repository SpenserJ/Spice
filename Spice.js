...
function SpiceRack(__) {
  this.version = '0.1';
  this.__ = this.TopiaObject = __; // This is what we'll call the original object
...
/**
 * Load an inventory
 * @param {Object} inventory Can be either an entity (Me), container (Tree), or inventory (Me.inventory)
...
SpiceRack.prototype.GetItemByType = function GetItemByType(itemtype) {
  if (typeof itemtype === 'undefined') return this.Abort("Spice.GetItemByType - Please provide an itemtype to search for.");
  ...
  for (var i = 0; i < items; i++) {
    if (inventory[i].itemtype == itemtype)
      return new Spice(inventory[i]);
  }
  return new Spice(null);
};
...
SpiceRack.prototype.ContainsItems = function ContainsItems(filter) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.ContainsItems - Please initialize Spice with an entity.");

  // If no filter is specified, allow everything
  if (typeof filter === 'undefined') filter = function() { return true; };

  return (this.__.inventory && // Has inventory
          Spice(this.__.inventory).Filter(function(item) { // Filter it's items
              return item.itemtype === 'wood'; // Does the inventory contain wood?
            }).__.length > 0); // Does it have at least one item of interest?
};
...
SpiceRack.prototype.MoveItem = function MoveItem(destination) {
  if (typeof this.__ === 'undefined') return new Spice();
...
SpiceRack.prototype.Equip = function Equip(destination) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Equip - Please initialize Spice with an item to equip.");
...
SpiceRack.prototype.Drop = function Drop(coordinates) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Drop - Please initialize Spice with an item to drop.");
...
SpiceRack.prototype.Loot = function Loot(filter) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Loot - Please initialize Spice with an entity to loot.");
  ...
  if (this.__IsNextTo(Me) === false)
    ...
    UseAction("Get", { id: this.__.id, slot: _.filter(this.__.inventory, filter)[0].slot });
  return true;
};
...
SpiceRack.prototype.Chop = function Chop() {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Chop - Please initialize Spice with an entity to chop.");
 ...
 * @param {Object} [target] Which target are we searching around? Defaults to Me
 */
SpiceRack.prototype.FindWithinDistance = function FindWithinDistance(distance, target) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.FindWithinDistance - Please initialize Spice with conditions for a Find(), or with {}.");
  
  if (typeof distance === 'undefined') distance = 10;
  if (typeof target   === 'undefined') target   = Me;

  var conditions = this.__;
  conditions.location = { "$within": { "$center": [[ target.x, target.y ], distance * 32 ]}};
  return new Spice(Find(conditions));
};

/**
 * Filter an array of entities
 * @param {Function} filter Function that returns true if the entity should be kept, and false if it should be filtered out
 */
SpiceRack.prototype.Filter = function Filter(filter) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Filter - Please initialize Spice with an array to filter.");
  if (typeof filter  === 'undefined') return this.Abort("Spice.Filter - Please provide a function to filter by.");

  return new Spice(_.filter(this.__, filter));
};
...
SpiceRack.prototype.Closest = function Closest(target) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Closest - Please initialize Spice with an array of entities.");

  if (typeof target === 'undefined') target = Me;

  // Set up the variables
  var matches = this.__, best_match = null, current_distance = 9999;

  // Loop through all of the matches, looking for the best
  for (var i = 0; i < matches.length; i++) {
    var examining = matches[i];

    // If this match is closer than our currently selected best_match, use this one instead
    if (Distance(examining, target) < current_distance) {
      best_match = examining;
      current_distance = Distance(examining, target);
    }
  }
  // If we've reached this point, we've checked one or more matches and determined that this is the best one
  return new Spice(best_match);
};
 ...
 * @param {Boolean} recursive Do you want to inspect the object recursively? [BROKEN]
 */
...
SpiceRack.prototype.Benchmark = function Benchmark(times) {
  if (typeof this.__ === 'undefined') return this.Abort("Spice.Benchmark - Please initialize Spice with a function to benchmark.");
 ...
 * @param {Object} target
 * @param {Object} [target2] You can specify an additional target directly, or use the already-loaded entity in this.__
 */
SpiceRack.prototype.__IsNextTo = function __IsNextTo(target, target2) {
  if (typeof target2 === 'undefined') {
    if (typeof this.__ === 'undefined') return this.Abort("Spice.__IsNextTo - You're missing an argument.");
    else target2 = this.__;
  }
